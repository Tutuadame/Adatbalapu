...
$id = $_GET["id"];
if(!empty($id)) {
	$sql = oci_parse($conn, "delete from CV where cv_id = $id");
...
echo '<h2>Az CV tábla adatai: </h2>';


$stid = oci_parse($conn, 'SELECT * FROM CV');
...
while ( $row = oci_fetch_array($stid, OCI_BOTH + OCI_RETURN_NULLS)) {
	echo '<div class="row record" style="display: flex; flex-wrap: wrap;">';
	     ...
	     echo '<div class="col-sm">';
	     	echo '<form action="cv.php?id='. $row[0]. '" method="post">';
