...
$email = $_GET["email"];
if(!empty($email)) {
	$sql = oci_parse($conn, "delete from Adminisztrator where email = '$email'");
	
	if(oci_execute($sql))
        {
                // Redirect to login page
                header("location: adminisztrator.php");
        } else{
...
echo '<h2>Az Adminisztrátor tábla adatai: </h2>';


$stid = oci_parse($conn, 'SELECT * FROM Adminisztrator');
...
while ( $row = oci_fetch_array($stid, OCI_BOTH + OCI_RETURN_NULLS)) {
	echo '<div class="row record" style="display: flex; flex-wrap: wrap;">';
	     ...
	     echo '<div class="col-sm">';
	     	echo '<form action="adminisztrator.php?email='. $row[0]. '" method="post">';
